...
`;
  const result = docker(['exec', '-i', name, 'psql', '-U', 'postgres', '-At', '-F', ',', '-c', sql]);
...
`;
  const result = docker(['exec', '-i', name, 'mysql', '-h127.0.0.1', '-uroot', '-N', '-B'], { input: sql });
...
`;
  const result = docker(['exec', '-i', name, '/opt/mssql-tools18/bin/sqlcmd', '-S', 'localhost', '-U', 'sa', '-P', 'Str0ngPass!234', '-C', '-W', '-h', '-1'], { input: sql });
...
exit
`;
  const result = docker(['exec', '-i', name, 'sqlplus', '-s', 'sqldesc/pass@//localhost/XEPDB1'], { input: sql });
