...
`;
  printSection(
    "sqlite cast runtime types",
    docker(["run", "--rm", "-i", "nouchka/sqlite3:latest"], { input: sql }),
  );
}
...
`;
  printSection(
    "duckdb cast metadata",
    docker(["run", "--rm", "-i", "duckdb/duckdb:latest"], { input: sql }),
  );
}
...
`;
  printSection(
    "postgres cast metadata",
    docker(["exec", "-i", name, "psql", "-U", "postgres", "-At", "-F", "|", "-c", sql]),
  );
}
...
`;
  printSection(
    "mysql cast metadata",
    docker(["exec", "-i", name, "mysql", "-h127.0.0.1", "-uroot", "-N", "-B"], { input: sql }),
  );
}
...
`;
  printSection(
    "oracle cast metadata",
    docker(["exec", "-i", name, "sqlplus", "-s", "sqldesc/pass@//localhost/XEPDB1"], {
      input: sql,
    }),
  );
}
