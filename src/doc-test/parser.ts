...
function splitLevel2Sections(markdown: string): Array<{ heading: string; title: string; body: string }> {
  const parts = markdown.split(/\n(?=## )/);
  return parts.slice(1).map((part) => {
    const [headingLine, ...bodyLines] = part.split('\n');
    const heading = headingLine.replace(/^## /, '').trim();
    const title = heading.replace(/^Prepare-\d+:\s*/, '').replace(/^TC-[\w-]+:\s*/, '').trim();
    return { heading, title, body: bodyLines.join('\n') };
  });
}

function parsePrepareSection(section: { heading: string; title: string; body: string }): PrepareBlock | undefined {
...
function parseGiven(body: string, defaultDialect: string): GivenSpec {
  ...
  if (typeof meta.prepare === 'string') {
    given.prepare = meta.prepare === 'none'
      ? undefined
      : meta.prepare.split(',').map((value) => value.trim()).filter(Boolean);
  ...
  if (!meta.prepare && !sql && !json && prose) {
    if (PROSE_PREPARE_NONE.test(prose)) {
      ...
      // handled above
    } else if (/スキーマファイル/.test(prose) || /loadSchema/.test(prose) || /describeQuery/.test(prose)) {
...
function parseWhen(body: string, defaultDialect: string): WhenSpec {
  ...
  if (sh && !sql) {
    return { kind: 'cli', dialect: typeof meta.dialect === 'string' ? meta.dialect : defaultDialect };
  }
...
function parseThen(body: string): ThenSpec {
  ...
  if (meta.kind === 'skip' || meta.automated === false) {
    return { kind: 'skip', target: 'first', verify, skipReason: typeof meta.reason === 'string' ? meta.reason : 'marked skip' };
  }

  const target: 'first' | 'last' = /最後の文/.test(prose) || meta.target === 'last' ? 'last' : 'first';
  ...
  if (meta.kind === 'error' || bullets.some((line) => /`Error`/.test(line))) {
    ...
    return {
      ...
      target,
      verify,
      errorMatch: typeof meta.match === 'string'
        ? meta.match
  ...
  if (
    meta.kind === 'none'
    || bullets.some((line) => /`columns`:\s*空/.test(line) || /結果なし/.test(line) || /結果列なし/.test(line))
  ) {
    const diagnostics = typeof meta.diagnostics === 'string'
      ? [meta.diagnostics]
      : bullets
    ...
    return {
      ...
      verify,
      diagnostics,
      resultKind: typeof meta.resultKind === 'string'
        ? meta.resultKind
        : resultKindMatch?.includes('none') ? 'none' : undefined,
    };
  }
