...
export function normalizeTypeName(value: string): string {
  ...
  const timestampWithLocalTimeZone = /^timestamp\s*\(([^)]+)\)\s+with\s+local\s+time\s+zone$/.exec(
    lower,
  );
  if (timestampWithLocalTimeZone?.[1])
    return `timestampltz(${timestampWithLocalTimeZone[1].trim()})`;
...
function parseParameterizedType(value: string): string | undefined {
  ...
  if (name === "nullable" || name === "lowcardinality")
    return args[0] ? normalizeTypeName(args[0]) : "unknown";
  if (
    [
      "char",
      "character",
  ...
  if (name === "varbit" || name === "bitvarying")
    return `bit varying(${args.map((arg) => arg.trim()).join(",")})`;
  if (name === "enum" || name === "set")
    return `${name}(${args.map((arg) => arg.trim()).join(",")})`;
  if (name === "array" || name === "list")
    return `array<${args[0] ? normalizeTypeName(args[0]) : "unknown"}>`;
  if (name === "map" && args.length >= 2)
    return `map<${normalizeTypeName(args[0])}, ${normalizeTypeName(args[1])}>`;
  if (name === "tuple" || name === "row")
    return `struct<${args
      .map((arg, index) => fieldFromTypeArgument(arg, index))
      .map((field) => `${field.name} ${field.type}`)
      .join(", ")}>`;
  if (name === "nested")
    return `array<struct<${args
      .map((arg, index) => fieldFromTypeArgument(arg, index))
      .map((field) => `${field.name} ${field.type}`)
      .join(", ")}>>`;
  return undefined;
