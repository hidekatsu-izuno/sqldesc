...
export function assertSupportedDialect(dialect?: string): string {
  ...
  if (!isSupportedDialect(normalized)) {
    throw new Error(
      `Unsupported SQL dialect "${dialect ?? normalized}". Run "sqldesc --dialects" to list supported dialects.`,
    );
  }
  return normalized;
}
...
export function adjustedOutputType(
...
export function configuredScalarFunctionPatternType(
  name: string,
  dialect: string,
): string | undefined {
  for (const [pattern, type] of Object.entries(
    getDialectConfig(dialect).scalarFunctionTypePatterns,
  )) {
...
export function commandResultColumnConfigs(
  command: string,
  dialect: string,
