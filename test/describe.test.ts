...
describe("describeQuery", () => {
  ...
  it("accepts common dialect aliases through the library API", async () => {
    ...
    assert.deepStrictEqual(
      sqliteResult.columns.map((column) => [column.name, column.type]),
      [["rowid", "integer"]],
    );
    ...
    assert.deepStrictEqual(
      sqlServerResult.columns.map((column) => [column.name, column.type]),
      [["id", "int"]],
    );

    const bigQueryResult = await describeQuery({
      sql: "select * except(age) from users",
      dialect: "bq",
      schema,
    });
    assert.deepStrictEqual(
      bigQueryResult.columns.map((column) => [column.name, column.type]),
      [
        ["id", "int64"],
        ["name", "string"],
      ],
  ...
  it("describes schema-qualified table references", async () => {
    ...
    const aliasResult = await describeQuery({
      sql: "select u.id, u.name from public.users as u",
      schema: qualifiedSchema,
    });
    assert.deepStrictEqual(
      aliasResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "public.users.id"],
    ...
    assert.deepStrictEqual(
      starResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "public.users.id"],
        ["name", "VARCHAR(255)", "public.users.name"],
      ],
  ...
  it("describes Oracle pseudocolumn result types", async () => {
    ...
    assert.deepStrictEqual(
      oracleResult.resultSets.map((set) =>
        set.columns.map((column) => [column.name, column.type, column.source]),
      ),
      [
        ...
        [
          ["cyc", "number(10)", "expression"],
          ["leaf", "number(10)", "expression"],
        ],
      ],
    );
  });
  ...
  it("marks columns from nullable outer-join sides as nullable", async () => {
    ...
    assert.deepStrictEqual(
      rightResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
        column.nullable,
    ...
    assert.deepStrictEqual(
      fullStarResult.columns.map((column) => [column.name, column.nullable]),
      [
        ["id", true],
        ["name", true],
        ["age", true],
  ...
  it("preserves table shapes with sampling and system time modifiers", async () => {
    ...
    assert.deepStrictEqual(
      sampleResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
    ...
    assert.deepStrictEqual(
      systemTimeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "users.id"],
  ...
  it("suppresses duplicate joined columns for using and natural joins", async () => {
    ...
    assert.deepStrictEqual(
      usingResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "users.id"],
        ["name", "VARCHAR(255)", "users.name"],
        ["age", "INTEGER", "users.age"],
    ...
    assert.deepStrictEqual(
      naturalResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "users.id"],
        ["name", "VARCHAR(255)", "users.name"],
        ["age", "INTEGER", "users.age"],
  ...
  it("honors star except, rename, and replace modifiers", async () => {
    const exceptResult = await describeQuery({
      sql: "select * except(age) from users",
      dialect: "bigquery",
      schema,
    });
    assert.deepStrictEqual(
      exceptResult.columns.map((column) => column.name),
      ["id", "name"],
    );

    const renameResult = await describeQuery({
      sql: "select * rename(name as full_name) from users",
      dialect: "bigquery",
      schema,
    });
    assert.deepStrictEqual(
      renameResult.columns.map((column) => [column.name, column.type]),
      [
        ["id", "int64"],
        ["full_name", "string"],
        ["age", "int64"],
    ...
    const replaceResult = await describeQuery({
      sql: "select * replace(42 as age) from users",
      dialect: "bigquery",
      schema,
    });
    assert.deepStrictEqual(
      replaceResult.columns.map((column) => [column.name, column.type]),
      [
        ["id", "int64"],
        ["name", "string"],
        ["age", "int64"],
  ...
  it("honors qualified star except modifiers", async () => {
    const tableResult = await describeQuery({
      sql: "select * except(users.age) from users",
      dialect: "bigquery",
      schema,
    });
    assert.deepStrictEqual(
      tableResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int64", "users.id"],
    ...
    const aliasResult = await describeQuery({
      sql: "select * except(u.age) from users u",
      dialect: "bigquery",
      schema,
    });
    assert.deepStrictEqual(
  ...
  it("honors table column aliases for qualified projections and stars", async () => {
    ...
    assert.deepStrictEqual(
      projectionResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes insert returning columns", async () => {
    ...
    assert.deepStrictEqual(
      fullValueResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      conflictResult.columns.map((column) => [column.name, column.type, column.source]),
      [["name", "text", "users.name"]],
    );
    ...
    assert.deepStrictEqual(
      excludedStarResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
        ["age", "integer", "users.age"],
  ...
  it("describes SQL Server output columns from DML statements", async () => {
    ...
    assert.deepStrictEqual(
      insertResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "users.id"],
        ["name", "nvarchar(max)", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      updateResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "users.id"],
        ["name", "nvarchar(max)", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      deleteResult.columns.map((column) => [column.name, column.type, column.source]),
      [["id", "int", "users.id"]],
    );
  });
  ...
  it("returns an empty result with warning for parsed non-result SQL", async () => {
    ...
    assert.partialDeepStrictEqual(result.statements[0], {
      kind: "create_table",
      resultKind: "none",
    });
    assert.partialDeepStrictEqual(result.diagnostics.at(-1), {
  ...
  it("classifies transaction session and maintenance statements as no-result SQL", async () => {
    const transactionResult = await describeQuery({
      sql: "begin; commit; rollback",
      dialect: "postgres",
    });
    assert.deepStrictEqual(transactionResult.columns, []);
    assert.deepStrictEqual(
      transactionResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["transaction", "none"],
        ["commit", "none"],
        ["rollback", "none"],
    ...
    assert.partialDeepStrictEqual(transactionResult.diagnostics.at(-1), {
      code: "SQLDESC_NO_RESULT_COLUMNS",
      severity: "info",
    });
    ...
    assert.deepStrictEqual(
      sessionResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["use", "none"],
        ["set_statement", "none"],
      ],
    ...
    const maintenanceResult = await describeQuery({
      sql: "analyze users; vacuum",
      dialect: "postgres",
    });
    assert.deepStrictEqual(
      maintenanceResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["analyze", "static"],
        ["command", "none"],
      ],
    ...
    const refreshResult = await describeQuery({
      sql: "refresh materialized view v",
      dialect: "postgres",
    });
    assert.partialDeepStrictEqual(refreshResult.statements[0], {
      kind: "refresh",
      resultKind: "none",
    });

    const truncateResult = await describeQuery({
      sql: "truncate table users",
      dialect: "postgres",
    });
    assert.partialDeepStrictEqual(truncateResult.statements[0], {
      kind: "truncate",
      resultKind: "none",
    });

    const lockResult = await describeQuery({ sql: "lock table users", dialect: "postgres" });
    assert.partialDeepStrictEqual(lockResult.statements[0], {
      kind: "command",
      resultKind: "none",
    });

    const reindexResult = await describeQuery({ sql: "reindex", dialect: "sqlite" });
    assert.partialDeepStrictEqual(reindexResult.statements[0], {
      kind: "column",
      resultKind: "none",
    });

    const msckResult = await describeQuery({ sql: "msck repair table users", dialect: "spark" });
    assert.partialDeepStrictEqual(msckResult.statements[0], {
      kind: "command",
      resultKind: "none",
    });

    const expressionCommandResult = await describeQuery({
      sql: "checkpoint; listen channel; notify channel; unlisten channel; savepoint s",
      dialect: "postgres",
    });
    assert.deepStrictEqual(expressionCommandResult.columns, []);
    assert.deepStrictEqual(
      expressionCommandResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["column", "none"],
        ["alias", "none"],
        ["alias", "none"],
  ...
  it("describes MySQL table maintenance status result columns", async () => {
    ...
    assert.partialDeepStrictEqual(analyzeResult.statements[0], {
      kind: "analyze",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      analyzeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.partialDeepStrictEqual(optimizeResult.statements[0], {
      kind: "command",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      optimizeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Table", "varchar(255)", "cast"],
        ["Op", "varchar(255)", "cast"],
        ["Msg_type", "varchar(255)", "cast"],
    ...
    const postgresAnalyzeResult = await describeQuery({
      sql: "analyze verbose users",
      dialect: "postgres",
    });
    assert.partialDeepStrictEqual(postgresAnalyzeResult.statements[0], {
      kind: "analyze",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      postgresAnalyzeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Table", "text", "cast"],
        ["Op", "text", "cast"],
        ["Msg_type", "text", "cast"],
  ...
  it("classifies additional DDL and access-control statements as no-result SQL", async () => {
    ...
    assert.deepStrictEqual(result.columns, []);
    assert.deepStrictEqual(
      result.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["create_index", "none"],
        ["drop_index", "none"],
        ["create_schema", "none"],
  ...
  it("classifies routine trigger and raw utility statements as no-result SQL", async () => {
    ...
    assert.deepStrictEqual(
      routineResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
    ...
    assert.deepStrictEqual(
      rawResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["raw", "none"],
        ["raw", "none"],
        ["raw", "none"],
    ...
    assert.partialDeepStrictEqual(rawResult.diagnostics.at(-1), {
      code: "SQLDESC_NO_RESULT_COLUMNS",
      severity: "info",
    });
    ...
    assert.deepStrictEqual(
      utilityResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["prepare", "none"],
        ["command", "none"],
        ["command", "none"],
  ...
  it("classifies object type namespace synonym and external object DDL as no-result SQL", async () => {
    ...
    assert.deepStrictEqual(
      objectResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["create_type", "none"],
        ["drop_type", "none"],
        ["create_type", "none"],
    ...
    const synonymResult = await describeQuery({
      sql: "create synonym s for users",
      dialect: "tsql",
    });
    assert.partialDeepStrictEqual(synonymResult.statements[0], {
      kind: "create_synonym",
      resultKind: "none",
    });

    const snowflakeResult = await describeQuery({
      sql: "create stage mystage; drop stage mystage; create file format ff type = csv; drop file format ff",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(
      snowflakeResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["raw", "none"],
        ["command", "none"],
        ["raw", "none"],
  ...
  it("tracks CREATE FUNCTION return types for later statements", async () => {
    ...
    assert.deepStrictEqual(
      result.resultSets[0]?.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      oracleResult.resultSets
        .at(-1)
        ?.columns.map((column) => [column.name, column.type, column.source]),
      [["n", "number", "function"]],
    );
    ...
    assert.deepStrictEqual(
      bigqueryExpressionBodyResult.resultSets
        .at(-1)
        ?.columns.map((column) => [column.name, column.type, column.source]),
      [["n", "int64", "function"]],
    );
    ...
    assert.deepStrictEqual(
      tableFunctionResult.resultSets
        .at(-1)
        ?.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      tableFunctionStarResult.resultSets
        .at(-1)
        ?.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "people.id"],
    ...
    const result = await describeQuery({
      ...
      schema: {
        tables: [
          {
            name: "users",
            columns: [{ name: "name", type: "VARCHAR(255)", nullable: false }],
          },
        ],
        functions: [
  ...
  it("describes values statements", async () => {
    ...
    assert.deepStrictEqual(
      nullableFirstRowResult.columns.map((column) => [column.name, column.type]),
      [
        ["column_1", "INTEGER"],
        ["column_2", "VARCHAR(255)"],
      ],
    ...
    assert.deepStrictEqual(
      widenedResult.columns.map((column) => [column.name, column.type]),
      [["column_1", "DECIMAL"]],
    );
    ...
    assert.deepStrictEqual(
      rowSyntaxResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "t.id"],
  ...
  it("describes singlestore json_extract_json and mysql-style convert", async () => {
    ...
    assert.deepStrictEqual(
      jsonExtractJsonResult.columns.map((column) => [column.name, column.type, column.source]),
      [["j", "json", "expression"]],
    );
    ...
    assert.deepStrictEqual(
      convertResult.columns.map((column) => [column.name, column.type, column.source]),
  ...
  it("describes starrocks get_json_string and bitmap_union_count", async () => {
    ...
    assert.deepStrictEqual(
      jsonResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      bitmapResult.columns.map((column) => [column.name, column.type, column.source]),
      [["buc", "int", "expression"]],
    );
  });
  ...
  it("describes teradata csum, otranslate, and hash functions", async () => {
    ...
    assert.deepStrictEqual(
      windowResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      stringResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      hashResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes set operations from the left query shape", async () => {
    ...
    assert.deepStrictEqual(
      nullableLeftResult.columns.map((column) => [column.name, column.type]),
      [["id", "INTEGER"]],
    );
    ...
    assert.deepStrictEqual(
      schemaBackedResult.columns.map((column) => [column.name, column.type]),
      [["id", "INTEGER"]],
    );
  ...
  it("honors explicit CTE column aliases", async () => {
    const result = await describeQuery({
      sql: "with q(one,label) as (select 1, 'a') select one, label from q",
    });
    assert.deepStrictEqual(
      result.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes scalar subquery projection types", async () => {
    ...
    assert.deepStrictEqual(
      correlatedResult.columns.map((column) => [column.name, column.type]),
      [
        ["id", "INTEGER"],
        ["order_count", "BIGINT"],
      ],
  ...
  it("describes table-valued function aliases with explicit columns", async () => {
    const result = await describeQuery({
      sql: "select f.id, f.name from my_func() as f(id, name)",
    });
    assert.deepStrictEqual(
      result.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const directSeriesResult = await describeQuery({
      sql: "select * from generate_series(1,3)",
      dialect: "postgres",
      schema,
    });
    assert.deepStrictEqual(
      directSeriesResult.columns.map((column) => [column.name, column.type, column.source]),
      [["generate_series", "integer", "generate_series.generate_series"]],
    );

    const sqliteSeriesResult = await describeQuery({
      sql: "select value from generate_series(1,3)",
      dialect: "sqlite",
      schema,
    });
    assert.deepStrictEqual(
      sqliteSeriesResult.columns.map((column) => [column.name, column.type, column.source]),
      [["value", "integer", "generate_series.value"]],
    );

    const oracleXplanResult = await describeQuery({
      sql: "select * from table(dbms_xplan.display)",
      dialect: "oracle",
    });
    assert.deepStrictEqual(
      oracleXplanResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      timestampResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      jsonEachResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["key", "text", "j.key"],
        ["value", "text", "j.value"],
      ],
    ...
    assert.deepStrictEqual(
      sqliteFtsHelperResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["o", "text", "expression"],
        ["m", "blob", "expression"],
      ],
    ...
    assert.deepStrictEqual(
      jsonEachStarResult.columns
        .slice(0, 4)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["key", "text", "j.key"],
        ["value", "text", "j.value"],
        ["type", "text", "j.type"],
    ...
    assert.deepStrictEqual(
      pgKeywordsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["word", "text", "k.word"],
    ...
    const clickHouseNumbersResult = await describeQuery({
      sql: "select * from numbers(3)",
      dialect: "clickhouse",
    });
    assert.deepStrictEqual(
      clickHouseNumbersResult.columns.map((column) => [column.name, column.type, column.source]),
      [["numbers", "INTEGER", "numbers.numbers"]],
    );

    const prestoSequenceResult = await describeQuery({
      sql: "select * from sequence(1,3) as t(x)",
      dialect: "presto",
    });
    assert.deepStrictEqual(
      prestoSequenceResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "integer", "t.x"]],
    );

    const pragmaTableInfoResult = await describeQuery({
      sql: "select * from pragma_table_info('users')",
      dialect: "duckdb",
    });
    assert.deepStrictEqual(
      pragmaTableInfoResult.columns
        .slice(0, 3)
        .map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes additional dialect table functions without expanding unrelated schema tables", async () => {
    ...
    assert.deepStrictEqual(
      jsonRecordsetResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["a", "integer", "x.a"],
        ["b", "text", "x.b"],
      ],
    ...
    assert.deepStrictEqual(
      regexpResult.columns.map((column) => [column.name, column.type, column.source]),
      [["m", "array<text>", "m.m"]],
    );
    ...
    assert.deepStrictEqual(
      subscriptsResult.columns.map((column) => [column.name, column.type, column.source]),
      [["i", "integer", "s.i"]],
    );
    ...
    assert.deepStrictEqual(
      splitTableResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "text", "x.x"]],
    );
    ...
    assert.deepStrictEqual(
      splitArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "array<text>", "x.x"]],
    );
    ...
    assert.deepStrictEqual(
      jsonArrayElementsResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "json", "x.x"]],
    );
    ...
    assert.deepStrictEqual(
      jsonEachTextResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["key", "text", "x.key"],
        ["value", "text", "x.value"],
      ],
    ...
    assert.deepStrictEqual(
      stringSplitResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      openQueryResult.columns.map((column) => [column.name, column.type, column.source]),
      [["id", "int", "openquery.id"]],
    );
    ...
    assert.deepStrictEqual(
      openRowsetResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "o.id"],
    ...
    assert.deepStrictEqual(
      duckDbReadBlobResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      duckDbReadTextResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      duckDbParquetSchemaResult.columns
        .slice(0, 4)
        .map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      snowflakeInferSchemaResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      clickHouseFileResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "BIGINT", "file.id"],
    ...
    assert.deepStrictEqual(
      bigQueryArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "int64", "x.x"]],
    );
    ...
    assert.deepStrictEqual(
      bigQueryDateArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [["d", "date", "d.d"]],
    );
    ...
    assert.deepStrictEqual(
      oracleCollectionResult.columns.map((column) => [column.name, column.type, column.source]),
      [["t", "varchar2(255)", "t.t"]],
    );
  });
  ...
  it("describes unnest aliases with explicit columns", async () => {
    ...
    const multiArrayResult = await describeQuery({
      sql: "select * from unnest(array[1,2], array['a','b']) as u(id, label)",
      dialect: "postgres",
    });
    assert.deepStrictEqual(
      multiArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "u.id"],
        ["label", "text", "u.label"],
      ],
    ...
    const ordinalityResult = await describeQuery({
      sql: "select * from unnest(array[1,2]) with ordinality as u(x, ord)",
      dialect: "postgres",
    });
    assert.deepStrictEqual(
      ordinalityResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["x", "integer", "u.x"],
        ["ord", "integer", "u.ord"],
      ],
  ...
  it("describes unnest aliases that expose the alias as a value column", async () => {
    const result = await describeQuery({
      sql: "select x from users, unnest(tags) as x",
      dialect: "bigquery",
      schema,
    });
    assert.deepStrictEqual(
    ...
    const typedResult = await describeQuery({
      sql: "select x from users, unnest(tags) as x",
      dialect: "bigquery",
      schema: arraySchema,
    });
    assert.deepStrictEqual(
      typedResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "string", "x.x"]],
    );
  });
  ...
  it("describes unaliased unnest over struct arrays", async () => {
    ...
    assert.deepStrictEqual(
      generatedArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "int64", "x.x"]],
    );
    ...
    assert.deepStrictEqual(
      mapResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["k", "varchar", "t.k"],
        ["v", "integer", "t.v"],
      ],
  ...
  it("describes lateral view aliases as generated columns", async () => {
    ...
    assert.deepStrictEqual(
      typedArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "VARCHAR(255)", "e.x"]],
    );
    ...
    assert.deepStrictEqual(
      arrayLiteralResult.columns.map((column) => [column.name, column.type, column.source]),
      [["x", "VARCHAR(255)", "e.x"]],
    );
    ...
    assert.deepStrictEqual(
      typedMapResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["k", "VARCHAR(255)", "e.k"],
        ["v", "INTEGER", "e.v"],
      ],
    ...
    assert.deepStrictEqual(
      posexplodeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["pos", "INTEGER", "e.pos"],
        ["x", "VARCHAR(255)", "e.x"],
      ],
  ...
  it("describes nested struct and array field projections from schema types", async () => {
    ...
    const fieldResult = await describeQuery({
      sql: "select profile.name from users",
      dialect: "bigquery",
      schema: nestedSchema,
    });
    ...
    const arrayFieldResult = await describeQuery({
      sql: "select addresses[offset(0)].city as city from users",
      dialect: "bigquery",
      schema: nestedSchema,
    });
    ...
    const arrayResult = await describeQuery({
      sql: "select scores[1] as first_score from users",
      dialect: "postgres",
      schema: nestedSchema,
    });
    ...
    const jsonExtractResult = await describeQuery({
      sql: "select json_extract(data, '$.name') as j, json_value(data, '$.name') as name from users",
      dialect: "mysql",
      schema: nestedSchema,
    });
    assert.deepStrictEqual(
      jsonExtractResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["j", "json", "expression"],
  ...
  it("describes json constructor expression result types", async () => {
    const mysqlResult = await describeQuery({
      sql: "select json_object('id', 1) as obj, json_array(1,2) as arr",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
    ...
    const postgresResult = await describeQuery({
      sql: "select json_build_object('id', 1) as obj, jsonb_build_object('id', 1) as objb, to_json('x') as scalar",
      dialect: "postgres",
    });
    assert.deepStrictEqual(
    ...
    const scalarResult = await describeQuery({
      sql: "select json_array_length('[1,2]') as len, json_typeof('{}') as kind",
      dialect: "postgres",
    });
    assert.deepStrictEqual(
      scalarResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      postgresPathResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      postgresJsonPathResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["jp", "jsonb", "expression"],
        ["ja", "jsonb", "expression"],
      ],
    ...
    assert.deepStrictEqual(
      bigQueryJsonResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sqliteJsonMutationResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes temporal function result types", async () => {
    ...
    assert.deepStrictEqual(
      postgresAgeResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      diffResult.columns.map((column) => [column.name, column.type, column.source]),
      [["days", "int", "expression"]],
    );
    ...
    assert.deepStrictEqual(
      timeZoneResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      arithmeticResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes conversion function result types", async () => {
    const conversionSchema: ValidationSchema = {
      tables: [
        {
          name: "t",
          columns: [{ name: "x", type: "VARCHAR(255)" }],
        },
      ],
    };
    ...
    assert.deepStrictEqual(
      tsqlResult.columns.map((column) => [column.name, column.type]),
      [
        ["a", "int"],
        ["b", "int"],
        ["c", "decimal(38, 10)"],
    ...
    assert.deepStrictEqual(
      bigQueryResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["s", "string", "polyglot"],
        ["d", "date", "expression"],
  ...
  it("describes identifier hash and random function result types", async () => {
    ...
    assert.deepStrictEqual(
      mixedResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      additionalHashResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sparkHashResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      mysqlNetworkResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes aggregate function result types", async () => {
    ...
    assert.deepStrictEqual(
      aliasAggregateResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sqliteJsonAggregateResult.columns.map((column) => [column.name, column.type, column.source]),
      [["a", "text", "expression"]],
    );
    ...
    assert.deepStrictEqual(
      tsqlAggregateResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes regex split and math function result types", async () => {
    ...
    assert.deepStrictEqual(
      sqliteResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["jd", "real", "expression"],
  ...
  it("describes row constructor expressions as records", async () => {
    const result = await describeQuery({
      sql: "select row(id, name) as r from users",
      dialect: "postgres",
      schema,
    });
  ...
  it("describes array map and struct constructor expressions", async () => {
    const postgresArray = await describeQuery({
      sql: "select array[1,2] as a",
      dialect: "postgres",
    });
    ...
    assert.deepStrictEqual(
      sparkConstructors.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const bigQueryStruct = await describeQuery({
      sql: "select struct(1 as id, 'a' as name) as s",
      dialect: "bigquery",
    });
  ...
  it("describes array scalar function result types", async () => {
    ...
    assert.deepStrictEqual(
      postgresArrayResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes map scalar function result types", async () => {
    const mapSchema: ValidationSchema = {
      tables: [
        {
          name: "users",
          columns: [{ name: "attrs", type: "map<text, integer>" }],
        },
      ],
    };
    const result = await describeQuery({
  ...
  it("describes common scalar function result types", async () => {
    ...
    assert.deepStrictEqual(
      dialectScalarResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      numericScalarResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      tsqlScalarResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes conditional expression result types", async () => {
    ...
    assert.deepStrictEqual(
      caseResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      ifResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      iffResult.columns.map((column) => [column.name, column.type, column.source]),
      [["label", "VARCHAR(255)", "expression"]],
    );
    ...
    assert.deepStrictEqual(
      nvl2Result.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      multiIfResult.columns.map((column) => [column.name, column.type, column.source]),
      [["label", "VARCHAR(255)", "expression"]],
    );
    ...
    assert.deepStrictEqual(
      chooseResult.columns.map((column) => [column.name, column.type, column.source]),
  ...
  it("describes predicate expression result types", async () => {
    ...
    assert.deepStrictEqual(
      dialectResult.columns.map((column) => [column.name, column.type]),
      [
        ["regex_match", "integer"],
        ["glob_match", "integer"],
        ["regexp_func", "integer"],
    ...
    assert.deepStrictEqual(
      mysqlFullTextResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("preserves projection shapes across select modifiers", async () => {
    ...
    assert.deepStrictEqual(
      distinctOnResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      qualifyResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int64", "users.id"],
        ["name", "string", "users.name"],
        ["rn", "int64", "expression"],
    ...
    const topResult = await describeQuery({
      sql: "select top 2 id, name from users",
      dialect: "tsql",
      schema,
    });
    assert.deepStrictEqual(
      topResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "users.id"],
        ["name", "nvarchar(max)", "users.name"],
      ],
    ...
    const fetchResult = await describeQuery({
      sql: "select id, name from users fetch first 2 rows only",
      dialect: "postgres",
      schema,
    });
    assert.deepStrictEqual(
      fetchResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
      ],
  ...
  it("describes grouping extension projections", async () => {
    ...
    assert.deepStrictEqual(
      rollupResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      cubeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["region", "varchar(255)", "orders.region"],
    ...
    assert.deepStrictEqual(
      groupingSetsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["region", "varchar(255)", "orders.region"],
        ["product", "varchar(255)", "orders.product"],
        ["total", "numeric", "expression"],
    ...
    assert.deepStrictEqual(
      groupingResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes json_table virtual columns", async () => {
    ...
    assert.deepStrictEqual(
      qualifiedResult.columns.map((column) => [column.name, column.type, column.source]),
      [["id", "int", "jt.id"]],
    );
    ...
    assert.deepStrictEqual(
      nestedResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["ord", "int", "jt.ord"],
  ...
  it("describes SQL Server openjson virtual columns", async () => {
    ...
    assert.deepStrictEqual(
      defaultResult.columns.map((column) => [column.name, column.type, column.source]),
      [
  ...
  it("describes oracle xmltable virtual columns", async () => {
    ...
    assert.deepStrictEqual(
      aggregateResult.columns.map((column) => [column.name, column.type, column.source]),
  ...
  it("describes snowflake lateral flatten columns", async () => {
    ...
    assert.deepStrictEqual(
      valueResult.columns.map((column) => [column.name, column.type, column.source]),
      [["value", "variant", "f.value"]],
    );
    ...
    assert.deepStrictEqual(
      tableFunctionResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["seq", "INTEGER", "f.seq"],
        ["key", "VARCHAR(255)", "f.key"],
        ["path", "VARCHAR(255)", "f.path"],
  ...
  it("describes snowflake generator and sequence functions", async () => {
    ...
    assert.deepStrictEqual(
      seqResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      getNextValResult.columns.map((column) => [column.name, column.type, column.source]),
  ...
  it("describes common metadata catalog projections without user schema", async () => {
    ...
    assert.deepStrictEqual(
      tablesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      columnsResult.columns.slice(0, 8).map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      pgCatalogResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      pgRolesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      pgLocksResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      constraintsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      oracleUsersResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      redshiftTablesResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      sysResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sysDmvResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sysDatabasesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sysRequestsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      sysIndexesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      systemTablesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      oracleVSessionResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      snowflakeAccountUsageResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      sqliteTempResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      bigQueryJobsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      snowflakeTableResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ...
        tables: [
          {
            ...
            name: "tables",
            columns: [{ name: "table_name", type: "VARCHAR(36)" }],
          },
        ],
      },
    });
    assert.deepStrictEqual(
      overriddenResult.columns.map((column) => [column.name, column.type, column.source]),
  ...
  it("describes CREATE definitions backed directly by VALUES", async () => {
    const viewResult = await describeQuery({
      sql: "create view value_view(label, n) as values ('a', 1)",
      dialect: "postgres",
    });
    assert.partialDeepStrictEqual(viewResult.statements[0], {
      kind: "create_view",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      viewResult.columns.map((column) => [column.name, column.type]),
      [
        ["label", "text"],
        ["n", "integer"],
      ],
    ...
    const tableResult = await describeQuery({
      sql: "create table value_table(label, n) as values ('a', 1)",
      dialect: "postgres",
    });
    assert.partialDeepStrictEqual(tableResult.statements[0], {
      kind: "create_table",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      tableResult.columns.map((column) => [column.name, column.type]),
      [
        ["label", "text"],
        ["n", "integer"],
      ],
    ...
    const trackedResult = await describeQuery({
      sql: "create view value_view(label, n) as values ('a', 1); select label, n from value_view",
      dialect: "postgres",
    });
    assert.deepStrictEqual(
      trackedResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["create_view", "static"],
        ["select", "static"],
      ],
  ...
  it("describes common show listing result columns", async () => {
    ...
    assert.deepStrictEqual(
      schemasResult.columns.map((column) => [column.name, column.type, column.source]),
      [["Schema", "text", "cast"]],
    );

    const variablesResult = await describeQuery({ sql: "show variables", dialect: "mysql" });
    assert.deepStrictEqual(
      variablesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const globalVariablesResult = await describeQuery({
      sql: "show global variables",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      globalVariablesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Variable_name", "varchar(255)", "cast"],
        ["Value", "varchar(255)", "cast"],
      ],
    ...
    assert.deepStrictEqual(
      warningsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      enginesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const engineStatusResult = await describeQuery({
      sql: "show engine innodb status",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      engineStatusResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      processlistResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Id", "int", "cast"],
    ...
    assert.deepStrictEqual(
      fullTablesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Table", "varchar(255)", "cast"],
    ...
    assert.deepStrictEqual(
      openTablesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      triggersResult.columns.slice(0, 5).map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      masterStatusResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      profilesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Query_ID", "int", "cast"],
    ...
    assert.deepStrictEqual(
      replicasResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Server_Id", "int", "cast"],
        ["Host", "varchar(255)", "cast"],
        ["Port", "int", "cast"],
    ...
    assert.deepStrictEqual(
      authorsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Name", "varchar(255)", "cast"],
    ...
    const createFunctionResult = await describeQuery({
      sql: "show create function f",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      createFunctionResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const functionCodeResult = await describeQuery({
      sql: "show function code f",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      functionCodeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Pos", "int", "cast"],
    ...
    const relayLogEventsResult = await describeQuery({
      sql: "show relaylog events",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      relayLogEventsResult.columns
        .slice(0, 3)
        .map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      pluginsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Name", "varchar(255)", "cast"],
        ["Status", "varchar(255)", "cast"],
        ["Type", "varchar(255)", "cast"],
    ...
    const functionStatusResult = await describeQuery({
      sql: "show function status",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      functionStatusResult.columns
        .slice(0, 5)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["Db", "varchar(255)", "cast"],
    ...
    const procedureStatusResult = await describeQuery({
      sql: "show procedure status",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      procedureStatusResult.columns
        .slice(0, 5)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["Db", "varchar(255)", "cast"],
    ...
    const snowflakeParametersResult = await describeQuery({
      sql: "show parameters",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(
      snowflakeParametersResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const snowflakeExternalTablesResult = await describeQuery({
      sql: "show external tables",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(
      snowflakeExternalTablesResult.columns
        .slice(0, 4)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["created_on", "TIMESTAMP", "cast"],
    ...
    const snowflakeSequencesResult = await describeQuery({
      sql: "show sequences",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(
      snowflakeSequencesResult.columns
        .slice(0, 4)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["created_on", "TIMESTAMP", "cast"],
    ...
    const snowflakeKeysResult = await describeQuery({
      sql: "show primary keys",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(
      snowflakeKeysResult.columns
        .slice(0, 4)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["created_on", "TIMESTAMP", "cast"],
    ...
    const snowflakeIntegrationsResult = await describeQuery({
      sql: "show integrations",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(
      snowflakeIntegrationsResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      catalogsResult.columns.map((column) => [column.name, column.type, column.source]),
      [["Catalog", "varchar", "cast"]],
    );

    const clickHouseShowDatabasesResult = await describeQuery({
      sql: "show databases",
      dialect: "clickhouse",
    });
    assert.deepStrictEqual(
      clickHouseShowDatabasesResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    const clickHouseDescribeResult = await describeQuery({
      sql: "describe table users",
      dialect: "clickhouse",
    });
    assert.deepStrictEqual(
      clickHouseDescribeResult.columns
        .slice(0, 3)
        .map((column) => [column.name, column.type, column.source]),
      [
        ["name", "VARCHAR(255)", "cast"],
    ...
    const partitionsResult = await describeQuery({
      sql: "show partitions users",
      dialect: "spark",
    });
    assert.deepStrictEqual(
      partitionsResult.columns.map((column) => [column.name, column.type, column.source]),
      [["partition", "VARCHAR(255)", "cast"]],
    );

    const tblPropertiesResult = await describeQuery({
      sql: "show tblproperties users",
      dialect: "spark",
    });
    assert.deepStrictEqual(
      tblPropertiesResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["key", "VARCHAR(255)", "cast"],
        ["value", "VARCHAR(255)", "cast"],
      ],
    ...
    const currentNamespaceResult = await describeQuery({
    ...
    const mysqlGrantsResult = await describeQuery({
      sql: "show grants for current_user",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      mysqlGrantsResult.columns.map((column) => [column.name, column.type, column.source]),
      [["Grants", "varchar(255)", "cast"]],
    );

    const mysqlCollationsResult = await describeQuery({ sql: "show collations", dialect: "mysql" });
    assert.deepStrictEqual(
      mysqlCollationsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    const trinoCreateSchemaResult = await describeQuery({
      sql: "show create schema default",
      dialect: "trino",
    });
    assert.deepStrictEqual(
      trinoCreateSchemaResult.columns.map((column) => [column.name, column.type, column.source]),
      [["Create Schema", "varchar", "cast"]],
    );

    const mysqlCreateDatabaseResult = await describeQuery({
      sql: "show create database mydb",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      mysqlCreateDatabaseResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Database", "varchar(255)", "cast"],
    ...
    const mysqlCreateSchemaResult = await describeQuery({
      sql: "show create schema mydb",
      dialect: "mysql",
    });
    assert.deepStrictEqual(
      mysqlCreateSchemaResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Database", "varchar(255)", "cast"],
        ["Create Database", "varchar(255)", "cast"],
      ],
  ...
  it("describes common show metadata result columns", async () => {
    ...
    assert.partialDeepStrictEqual(columnsResult.statements[0], {
      kind: "show",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      columnsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      createResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Table", "varchar(255)", "cast"],
    ...
    assert.deepStrictEqual(
      indexResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["Table", "varchar(255)", "cast"],
        ["Non_unique", "int", "cast"],
  ...
  it("describes explain query targets and describe table targets when schema is available", async () => {
    ...
    assert.partialDeepStrictEqual(describeResult.statements[0], {
      kind: "describe",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      describeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "users.id"],
        ["name", "VARCHAR(255)", "users.name"],
        ["age", "INTEGER", "users.age"],
    ...
    assert.partialDeepStrictEqual(explainResult.statements[0], {
      kind: "describe",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      explainResult.columns.map((column) => [column.name, column.type, column.source]),
      [["QUERY PLAN", "VARCHAR(255)", "cast"]],
    );

    const clickHouseExplainResult = await describeQuery({
      sql: "explain ast select id from users",
      dialect: "clickhouse",
      schema,
    });
    assert.partialDeepStrictEqual(clickHouseExplainResult.statements[0], {
      kind: "command",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      clickHouseExplainResult.columns.map((column) => [column.name, column.type, column.source]),
      [["QUERY PLAN", "VARCHAR(255)", "cast"]],
    );

    const explainAnalyzeResult = await describeQuery({
      sql: "explain analyze select id from users",
      dialect: "postgres",
      schema,
    });
    assert.deepStrictEqual(
      explainAnalyzeResult.columns.map((column) => [column.name, column.type, column.source]),
      [["QUERY PLAN", "text", "cast"]],
    );

    const mysqlExplainResult = await describeQuery({
      sql: "explain select id from users",
      dialect: "mysql",
      schema,
    });
    assert.deepStrictEqual(
      mysqlExplainResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "cast"],
    ...
    const sqliteExplainResult = await describeQuery({
      sql: "explain select id from users",
      dialect: "sqlite",
      schema,
    });
    assert.deepStrictEqual(
      sqliteExplainResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["addr", "integer", "cast"],
        ["opcode", "text", "cast"],
        ["p1", "integer", "cast"],
    ...
    const sparkDescribeFunctionResult = await describeQuery({
      sql: "describe function abs",
      dialect: "spark",
      schema,
    });
    assert.deepStrictEqual(
      sparkDescribeFunctionResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    const snowflakeDescribeWarehouseResult = await describeQuery({
      sql: "describe warehouse wh",
      dialect: "snowflake",
      schema,
    });
    assert.deepStrictEqual(
      snowflakeDescribeWarehouseResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
  ...
  it("reports non-static statement diagnostics even when other statements are static", async () => {
    ...
    assert.deepStrictEqual(
      runtimeResult.columns.map((column) => [column.name, column.type]),
      [["one", "INTEGER"]],
    );
    assert.deepStrictEqual(
      runtimeResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["select", "static"],
        ["command", "runtime"],
      ],
    ...
    assert.ok(
      runtimeResult.diagnostics.some((entry) =>
        matchesPartial(entry, { code: "SQLDESC_RUNTIME_RESULT_SHAPE", severity: "warning" }),
      ),
    );
  });
  ...
  it("classifies runtime-dependent result shapes", async () => {
    ...
    assert.partialDeepStrictEqual(executeResult.statements[0], {
      kind: "execute",
      resultKind: "runtime",
    });
    assert.partialDeepStrictEqual(executeResult.diagnostics.at(-1), {
      code: "SQLDESC_RUNTIME_RESULT_SHAPE",
    });

    const tsqlExecuteResult = await describeQuery({ sql: "exec dbo.my_proc", dialect: "tsql" });
    assert.partialDeepStrictEqual(tsqlExecuteResult.statements[0], {
      kind: "execute",
      resultKind: "runtime",
    });
    assert.ok(
      tsqlExecuteResult.diagnostics.some((entry) =>
        matchesPartial(entry, { code: "SQLDESC_RUNTIME_RESULT_SHAPE", severity: "warning" }),
      ),
    );

    const copyResult = await describeQuery({
      sql: "copy unknown_table to '/tmp/x.csv'",
      dialect: "postgres",
    });
    assert.partialDeepStrictEqual(copyResult.statements[0], {
      kind: "copy",
      resultKind: "runtime",
    });
    ...
    assert.partialDeepStrictEqual(dbccResult.statements[0], {
      kind: "alias",
      resultKind: "runtime",
    });
    assert.ok(
      dbccResult.diagnostics.some((entry) =>
        matchesPartial(entry, { code: "SQLDESC_RUNTIME_RESULT_SHAPE" }),
      ),
    );
    assert.ok(
      !dbccResult.diagnostics.some((entry) =>
        /Invalid expression|Unexpected token/i.test(entry.message),
      ),
    );

    const copyFromResult = await describeQuery({
      sql: "copy users from stdin",
      dialect: "postgres",
      schema,
    });
    assert.deepStrictEqual(copyFromResult.columns, []);
    assert.partialDeepStrictEqual(copyFromResult.statements[0], {
      kind: "copy",
      resultKind: "none",
    });
  });
  ...
  it("describes copy and export query shapes", async () => {
    ...
    assert.deepStrictEqual(
      copyQuery.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      copyTable.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
        ["age", "integer", "users.age"],
    ...
    assert.deepStrictEqual(
      snowflakeCopy.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "users.id"],
        ["name", "VARCHAR(255)", "users.name"],
      ],
    ...
    assert.partialDeepStrictEqual(exportResult.statements[0], {
      kind: "export",
      resultKind: "static",
    });
    assert.deepStrictEqual(
      exportResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int64", "users.id"],
        ["name", "string", "users.name"],
      ],
  ...
  it("describes execute result shapes from earlier prepare statements", async () => {
    ...
    assert.deepStrictEqual(
      deallocatedResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["prepare", "none"],
        ["execute", "static"],
        ["command", "none"],
    ...
    assert.deepStrictEqual(
      deallocatedResult.resultSets.map((resultSet) => resultSet.index),
      [2],
    );
    assert.ok(
      deallocatedResult.diagnostics.some((entry) =>
        matchesPartial(entry, {
          code: "SQLDESC_RUNTIME_RESULT_SHAPE",
          message:
            "EXECUTE parses successfully, but its result-set shape depends on runtime database behavior.",
        }),
      ),
    );

    const spHelpResult = await describeQuery({
      sql: "exec sp_help users",
      dialect: "tsql",
      schema,
    });
    assert.deepStrictEqual(
      spHelpResult.columns.map((column) => [column.name, column.type, column.source]).slice(0, 4),
      [
    ...
    assert.deepStrictEqual(
      spWhoResult.columns.map((column) => [column.name, column.type, column.source]).slice(0, 4),
      [
        ["spid", "int", "cast"],
        ["ecid", "int", "cast"],
    ...
    const spSpaceUsedResult = await describeQuery({
      sql: "exec sp_spaceused users",
      dialect: "tsql",
      schema,
    });
    assert.deepStrictEqual(
      spSpaceUsedResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["name", "nvarchar(max)", "cast"],
    ...
    assert.deepStrictEqual(
      declaredResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "cast"],
  ...
  it("describes calls to procedures defined earlier in the same SQL", async () => {
    ...
    assert.deepStrictEqual(
      tsqlResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["create_procedure", "none"],
        ["execute", "static"],
      ],
    ...
    assert.deepStrictEqual(
      tsqlResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "int", "users.id"],
        ["name", "nvarchar(max)", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      mysqlResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["create_procedure", "none"],
        ["command", "static"],
      ],
    ...
    assert.deepStrictEqual(
      snowflakeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "DECIMAL", "cast"],
        ["name", "VARCHAR(255)", "cast"],
      ],
    ...
    assert.deepStrictEqual(
      droppedProcedureResult.resultSets.map((resultSet) => resultSet.index),
      [2],
    );
    assert.deepStrictEqual(
      droppedProcedureResult.statements.map((statement) => [statement.kind, statement.resultKind]),
      [
        ["create_procedure", "none"],
        ["command", "static"],
        ["drop_procedure", "none"],
    ...
    assert.ok(
      droppedProcedureResult.diagnostics.some((entry) =>
        matchesPartial(entry, {
          code: "SQLDESC_RUNTIME_RESULT_SHAPE",
          severity: "warning",
        }),
  ...
  it("classifies non-result statements", async () => {
    const result = await describeQuery({
      sql: "merge into users using src on users.id = src.id when matched then update set name = src.name",
    });
    assert.deepStrictEqual(result.columns, []);
    ...
    const declareResult = await describeQuery({
      sql: "declare c cursor for select id from users",
      dialect: "postgres",
    });
    assert.deepStrictEqual(declareResult.columns, []);
    assert.partialDeepStrictEqual(declareResult.statements[0], {
      kind: "declare",
      resultKind: "none",
    });

    const triggerResult = await describeQuery({
      sql: "create trigger tr before insert on users execute function f()",
      dialect: "postgres",
    });
    assert.deepStrictEqual(triggerResult.columns, []);
    assert.partialDeepStrictEqual(triggerResult.statements[0], {
      kind: "create_trigger",
      resultKind: "none",
    });

    const attachResult = await describeQuery({
      sql: "attach database 'x.db' as x",
      dialect: "sqlite",
    });
    assert.deepStrictEqual(attachResult.columns, []);
    assert.partialDeepStrictEqual(attachResult.statements[0], {
      kind: "attach",
      resultKind: "none",
    });
    ...
    assert.partialDeepStrictEqual(detachResult.statements[0], {
      kind: "detach",
      resultKind: "none",
    });
    ...
    assert.partialDeepStrictEqual(uncacheResult.statements[0], {
      kind: "uncache",
      resultKind: "none",
    });

    const alterIndexResult = await describeQuery({
      sql: "alter index idx rename to idx2",
      dialect: "postgres",
    });
    assert.deepStrictEqual(alterIndexResult.columns, []);
    assert.partialDeepStrictEqual(alterIndexResult.statements[0], {
      kind: "alter_index",
      resultKind: "none",
    });

    const taskResult = await describeQuery({
      sql: "create task t as select 1",
      dialect: "snowflake",
    });
    assert.deepStrictEqual(taskResult.columns, []);
    assert.partialDeepStrictEqual(taskResult.statements[0], {
      kind: "create_task",
      resultKind: "none",
    });
  });
  ...
  it("preserves multiple result sets for multi-statement SQL", async () => {
    ...
    assert.deepStrictEqual(
      result.resultSets.map((set) => set.columns.map((column) => [column.name, column.type])),
      [
        [["one", "INTEGER"]],
        [
          ["column_1", "INTEGER"],
    ...
    assert.deepStrictEqual(
      blockResult.resultSets.map((set) => [
        set.index,
        set.columns.map((column) => [column.name, column.type, column.source]),
      ]),
      [
  ...
  it("uses definitions from earlier statements in later statement schemas", async () => {
    const viewResult = await describeQuery({
      sql: "create view v as select 1 as id; select id from v",
    });
    assert.deepStrictEqual(
      viewResult.resultSets[1]?.columns.map((column) => [column.name, column.type, column.source]),
      [["id", "INTEGER", "v.id"]],
    );
    ...
    assert.deepStrictEqual(
      schemaBackedViewResult.resultSets[1]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.ok(
      !schemaBackedViewResult.diagnostics.some((entry) => matchesPartial(entry, { code: "E200" })),
    );
    ...
    assert.deepStrictEqual(
      rawRecursiveViewResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      sparkGlobalViewResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      duckDbTableMacroResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      duckDbAliasedTableMacroResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      duckDbColumnAliasedTableMacroResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    const ctasResult = await describeQuery({
      sql: "create table t as select 1 as id, 'a' as label; select label from t",
    });
    assert.deepStrictEqual(
      ctasResult.resultSets[1]?.columns.map((column) => [column.name, column.type, column.source]),
      [["label", "VARCHAR(255)", "t.label"]],
    );

    const tableResult = await describeQuery({
      sql: "create table local_users (id int, name text); select id, name from local_users",
    });
    assert.strictEqual(tableResult.resultSets[0]?.index, 2);
    assert.deepStrictEqual(
      tableResult.resultSets[0]?.columns.map((column) => [column.name, column.type, column.source]),
      [
    ...
    assert.deepStrictEqual(
      clickHouseNestedResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.deepStrictEqual(
      synonymResult.resultSets[0]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
  ...
  it("keeps schema-qualified and materialized view definitions in local schema", async () => {
    ...
    assert.deepStrictEqual(
      result.resultSets[1]?.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      renameResult.resultSets[1]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
  ...
  it("applies earlier alter table actions to later statement schemas", async () => {
    ...
    assert.deepStrictEqual(
      addResult.resultSets[0]?.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "INTEGER", "users.id"],
        ["name", "VARCHAR(255)", "users.name"],
      ],
    ...
    assert.deepStrictEqual(
      dropResult.columns.map((column) => [column.name, column.type, column.source]),
      [["id", "INTEGER", "users.id"]],
    );
    ...
    assert.deepStrictEqual(
      renameResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      alterTypeResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      changeResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      modifyResult.columns.map((column) => [column.name, column.type, column.source]),
      [["age", "bigint", "users.age"]],
    );
    ...
    assert.partialDeepStrictEqual(notNullResult.columns[0], {
      name: "name",
      type: "text",
      nullable: false,
    });
    ...
    assert.deepStrictEqual(
      addColumnsResult.columns.map((column) => [column.name, column.type, column.source]),
      [
        ["id", "integer", "users.id"],
        ["name", "text", "users.name"],
        ["age", "integer", "users.age"],
    ...
    assert.deepStrictEqual(
      serialResult.columns.map((column) => [
        column.name,
        column.type,
        column.source,
        column.nullable,
  ...
  it("applies earlier drop and table rename actions to later statement schemas", async () => {
    ...
    assert.deepStrictEqual(
      dropResult.resultSets.map((resultSet) => resultSet.index),
      [],
    );
    assert.partialDeepStrictEqual(dropResult.statements[2], {
      kind: "select",
      resultKind: "unknown",
    });
    assert.partialDeepStrictEqual(dropResult.diagnostics.at(-1), {
    ...
    assert.deepStrictEqual(
      dropViewResult.resultSets.map((resultSet) => resultSet.index),
      [1],
    );
    ...
    assert.deepStrictEqual(
      dropSchemaResult.resultSets.map((resultSet) => resultSet.index),
      [],
    );
    assert.partialDeepStrictEqual(dropSchemaResult.statements[2], {
      kind: "select",
      resultKind: "unknown",
    });
    ...
    assert.deepStrictEqual(
      dropNamespaceResult.resultSets.map((resultSet) => resultSet.index),
      [],
    );
    assert.partialDeepStrictEqual(dropNamespaceResult.statements[2], {
      kind: "select",
      resultKind: "unknown",
    });
    ...
    assert.deepStrictEqual(
      setSchemaResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      renameSchemaResult.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      renameViewResult.resultSets[1]?.columns.map((column) => [
        column.name,
        column.type,
        column.source,
      ]),
    ...
    assert.partialDeepStrictEqual(renameViewResult.statements[1], {
      kind: "alter_view",
      resultKind: "none",
    });
  });
  ...
  it("uses select into targets as local schema for later statements", async () => {
    ...
    assert.deepStrictEqual(
      result.resultSets.map((resultSet) => resultSet.index),
      [1, 2],
    );
    assert.deepStrictEqual(
      result.resultSets[1]?.columns.map((column) => [column.name, column.type, column.source]),
    ...
    assert.deepStrictEqual(
      tempResult.resultSets[1]?.columns.map((column) => [column.name, column.type, column.source]),
      [["id", "int", "#ids.id"]],
    );
  });
  ...
  it("uses create table like and clone sources as local schema for later statements", async () => {
    ...
    assert.deepStrictEqual(
      likeResult.columns.map((column) => [column.name, column.type, column.source]),
      [
