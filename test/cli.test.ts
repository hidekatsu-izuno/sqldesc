...
describe("sqldesc CLI", () => {
  ...
  it("prints empty strings for dynamic result column names in JSON output", async () => {
    ...
    assert.deepStrictEqual(
      json.columns.map((column: Record<string, unknown>) => [column.name, column.type]),
      [["", "nvarchar(max)"]],
    );
  });
  ...
  it("returns structured diagnostics for metadata-dependent SQL in JSON output", async () => {
    ...
    assert.strictEqual(result.stderr, "");
    const json = JSON.parse(result.stdout);
    assert.partialDeepStrictEqual(json.columns, [{ name: "one", type: "INTEGER" }]);
    assert.partialDeepStrictEqual(json.statements, [
  ...
  it("prints runtime-dependent result-shape diagnostics in text output", async () => {
    ...
    assert.match(
      result.stderr,
      /warning: SQLDESC_RUNTIME_RESULT_SHAPE: COMMAND parses successfully/,
    );
    assert.strictEqual(
      result.stderr.match(
        /COMMAND parses successfully, but its result-set shape depends on runtime database behavior\./g,
      )?.length,
      1,
    );
  });
  ...
  it("prints metadata-dependent result-shape diagnostics once in text output", async () => {
    ...
    assert.match(
      result.stderr,
      /warning: SQLDESC_METADATA_RESULT_SHAPE: DESCRIBE parses successfully/,
    );
    assert.strictEqual(
      result.stderr.match(
        /DESCRIBE parses successfully, but its result-set shape is dialect-specific metadata and cannot be inferred statically\./g,
      )?.length,
      1,
    );
  });
