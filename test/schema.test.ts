...
describe("parseCreateTables", () => {
  ...
  it("accepts common dialect aliases while parsing schema SQL", () => {
    assert.deepStrictEqual(
      parseCreateTables("create table dbo.t ([id] int identity(1,1) primary key)", "sqlserver"),
      [
        {
          name: "t",
          schema: "dbo",
...
describe("parseCreateViews", () => {
  it("extracts view columns from schema-backed projections", () => {
    ...
    assert.deepStrictEqual(
      parseCreateViews("create view active_users as select id, name from users", baseSchema),
      [
        {
          name: "active_users",
          columns: [
  ...
  it("uses explicit view columns and set operation output shape", () => {
    assert.deepStrictEqual(parseCreateViews("create view numbers(n) as select 1 union select 2"), [
      {
        name: "numbers",
        columns: [{ name: "n", type: "integer", nullable: undefined }],
      },
    ]);

    assert.deepStrictEqual(
      parseCreateViews(
        "create materialized view v(a, b) as select * from users",
        {
          tables: [
            {
              name: "users",
  ...
  it("extracts VALUES-backed view and CTAS columns when Polyglot omits CREATE AST", () => {
    assert.deepStrictEqual(
      parseCreateViews(
        "create view value_view(label, n) as values ('a', 1), ('b', 2)",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateAsTables(
        "create table value_table(label, n) as values ('a', 1)",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view semicolon_value(label, n) as values ('a;b', 1);",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view derived_value_view(label, n) as select * from (values ('a', 1)) as x(a, b)",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateAsTables(
        "create table derived_value_table(label, n) as select * from (values ('a', 1)) as x(a, b)",
        undefined,
        "postgres",
      ),
      [
  ...
  it("extracts cast expression types from view queries", () => {
    assert.deepStrictEqual(
      parseCreateViews(
        "create view typed_view as select id::int as id, name::text as name from users",
        {
          tables: [
            {
              name: "users",
  ...
  it("resolves CTE-backed view projection types", () => {
    assert.deepStrictEqual(
      parseCreateViews(
        "create view cte_view as with q as (select 1 as id) select id from q",
        undefined,
        "postgres",
      ),
      [
        {
          name: "cte_view",
          columns: [{ name: "id", type: "integer", nullable: undefined }],
        },
      ],
    );
  });
  ...
  it("resolves derived table and table function view projection types", () => {
    assert.deepStrictEqual(
      parseCreateViews(
        "create view derived_view as select x.id from (select 1 as id) x",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view derived_star_view as select * from (select 1 as id, 'a' as name) x",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view series_view as select g.n from generate_series(1,3) as g(n)",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view json_each_view as select j.* from json_each('{\"a\":1}') as j",
        undefined,
        "sqlite",
      )[0]?.columns.slice(0, 4),
      [
  ...
  it("expands joined star projections and suppresses USING/NATURAL duplicate columns", () => {
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view joined_view as select * from users join orders using(id)",
        joinSchema,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view natural_view as select * from users natural join orders",
        joinSchema,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view left_join_view as select users.id, orders.total from users left join orders on users.id = orders.id",
        joinSchema,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateViews(
        "create view full_join_view as select * from users full join orders on users.id = orders.id",
        joinSchema,
        "postgres",
      ),
      [
  ...
  it("preserves schema-qualified materialized view names", () => {
    assert.deepStrictEqual(
      parseCreateViews("create materialized view public.user_ids as select 1 as id"),
      [
        {
          name: "user_ids",
          schema: "public",
...
describe("parseCreateAsTables", () => {
  it("extracts create table as select columns", () => {
    assert.deepStrictEqual(
      parseCreateAsTables("create table generated_users as select 1 as id, 'a' as name"),
      [
        {
  ...
  it("prefers explicit create table column definitions", () => {
    assert.deepStrictEqual(
      parseCreateAsTables("create table generated_users (id int) as select 1"),
      [
        {
          name: "generated_users",
          columns: [
  ...
  it("copies create table like and clone columns from existing schema", () => {
    ...
    assert.deepStrictEqual(
      parseCreateAsTables("create table new_users (like users)", baseSchema, "postgres"),
      [
        {
          name: "new_users",
          columns: [
    ...
    assert.deepStrictEqual(
      parseCreateAsTables(
        "create table expanded_users (like users, age int)",
        baseSchema,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateAsTables("create table cloned_users clone users", baseSchema, "snowflake"),
      [
        {
          name: "cloned_users",
          columns: [
  ...
  it("preserves schema-qualified create table as names", () => {
    assert.deepStrictEqual(
      parseCreateAsTables("create table public.generated_users as select 1 as id"),
      [
        {
          name: "generated_users",
          schema: "public",
  ...
  it("resolves CTE-backed create table as projection types", () => {
    assert.deepStrictEqual(
      parseCreateAsTables(
        "create table cte_table as with q as (select 1 as id) select id from q",
        undefined,
        "postgres",
      ),
      [
  ...
  it("resolves derived table create table as projection types", () => {
    assert.deepStrictEqual(
      parseCreateAsTables(
        "create table derived_table as select x.id from (select 1 as id) x",
        undefined,
        "postgres",
      ),
      [
    ...
    assert.deepStrictEqual(
      parseCreateAsTables(
        "create table derived_star_table as select * from (select 1 as id, 'a' as name) x",
        undefined,
        "postgres",
      ),
      [
...
describe("parseCreateSynonyms", () => {
  ...
  it("preserves schema-qualified synonym names and targets", () => {
    ...
    assert.deepStrictEqual(
      parseCreateSynonyms("create synonym app.user_syn for public.users", baseSchema),
      [
        {
          schema: "app",
          name: "user_syn",
...
describe("loadSchema", () => {
  it("loads Oracle SQL*Plus schema scripts split by slash commands", async () => {
    ...
    await writeFile(
      path.join(cwd, "schemas", "schema.sql"),
      `
      set define off
    ...
    assert.deepStrictEqual(
      schema.tables
        .find((table) => table.name === "user_names")
        ?.columns.map((column) => [column.name, column.type]),
      [
        ["id", "decimal"],
        ["name", "varchar2(20)"],
  ...
  it("loads Oracle SQL*Plus scripts included with @ commands", async () => {
    ...
    await writeFile(
      path.join(cwd, "schemas", "parts", "tables.sql"),
      `
  ...
  it("loads Oracle SQL*Plus scripts with DEFINE substitution", async () => {
    ...
    assert.deepStrictEqual(
      schema.tables
        .find((table) => table.name === "users" && table.schema === "APP")
        ?.columns.map((column) => [column.name, column.type]),
      [
        ["id", "decimal"],
        ["name", "varchar2(20)"],
    ...
    assert.deepStrictEqual(
      schema.tables
        .find((table) => table.name === "user_names" && table.schema === "APP")
        ?.columns.map((column) => [column.name, column.type]),
      [
        ["id", "decimal"],
        ["name", "varchar2(20)"],
  ...
  it("honors literal PostgreSQL psql conditional blocks while loading schema scripts", async () => {
    ...
    const schema = await loadSchema(["schemas/schema.sql"], { cwd, dialect: "postgres" });
    assert.strictEqual(
      schema.tables.some((table) => table.name === "skipped"),
      false,
    );
    assert.deepStrictEqual(
      schema.tables
  ...
  it("loads sqlcmd scripts with :setvar variable substitution", async () => {
    ...
    assert.deepStrictEqual(
      schema.tables
        .find((table) => table.name === "user_names" && table.schema === "dbo")
        ?.columns.map((column) => [column.name, column.type]),
      [
        ["id", "integer"],
        ["name", "nvarchar(20)"],
  ...
  it("applies alter table actions before dependent schema objects", async () => {
    ...
    assert.deepStrictEqual(
      schema.tables
        .find((table) => table.name === "user_view")
        ?.columns.map((column) => [column.name, column.type]),
      [
        ["id", "integer"],
        ["age", "bigint"],
  ...
  it("applies drop and schema rename mutations from schema files", async () => {
    ...
    const schema = await loadSchema(["schemas/**/*.sql"], { cwd, dialect: "postgres" });
    assert.strictEqual(
      schema.tables.some((table) => table.name === "stale"),
      false,
    );
    assert.strictEqual(
      schema.tables.some((table) => table.name === "stale_view"),
      false,
    );
    assert.partialDeepStrictEqual(
      schema.tables.find((table) => table.name === "events"),
      { schema: "archive" },
    );
    assert.deepStrictEqual(
      schema.tables
        .find((table) => table.name === "event_ids")
        ?.columns.map((column) => [column.name, column.type]),
      [["id", "integer"]],
    );
  });
  ...
  it("resolves user-defined type aliases across schema files", async () => {
    ...
    await mkdir(path.join(cwd, "schemas"), { recursive: true });
    await writeFile(
      path.join(cwd, "schemas", "00-types.sql"),
      [
        "create domain positive_int as int check (value > 0)",
        "create type mood as enum ('sad','ok','happy')",
        "create type pair as (id int, name text)",
    ...
    await writeFile(
      path.join(cwd, "schemas", "10-table.sql"),
      "create table t(id positive_int, m mood, p pair);",
    );
  ...
  it("resolves procedure result columns against views from later schema files", async () => {
    ...
    await mkdir(path.join(cwd, "schemas"), { recursive: true });
    await writeFile(
      path.join(cwd, "schemas", "00-table.sql"),
      "create table users(id int, name text);",
    );
    await writeFile(
      path.join(cwd, "schemas", "10-procedure.sql"),
      "create procedure p() begin select id, name from v; end;",
    );
    await writeFile(
      path.join(cwd, "schemas", "20-view.sql"),
  ...
  it("uses scalar function return types while loading procedure result columns", async () => {
    ...
    assert.deepStrictEqual(schema.procedures, [
      {
        name: "p",
        columns: [{ name: "score", type: "integer", nullable: undefined }],
      },
    ]);
  });
  ...
  it("applies drop function and drop procedure routine mutations from schema files", async () => {
    ...
    const schema = await loadSchema(["schemas/**/*.sql"], { cwd, dialect: "mysql" });
    assert.deepStrictEqual(schema.functions, [{ name: "label", returnType: "integer" }]);
    assert.deepStrictEqual(schema.procedures, [
      {
        name: "p",
        columns: [{ name: "name", type: "text", nullable: undefined }],
      },
    ]);
  });
});
