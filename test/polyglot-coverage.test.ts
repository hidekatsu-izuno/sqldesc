...
const staticResultCases = [
  ...
  ["teradata", "select id, name from users sample 10"],
  ["teradata", "select zeroifnull(age) z, nullifzero(age) nz from users"],
  [
    "teradata",
    "select csum(age) over (order by id) csum, mavg(age) over (order by id rows 3 preceding) mavg from users",
  ],
  ["teradata", "select otranslate(name,'abc','xyz') ot, oreplace(name,'a','b') orp from users"],
  ["teradata", "select hashamp(id) ha, hashbucket(id) hb, index(name) idx from users"],
  ["teradata", "select id from users minus select user_id as id from orders"],
  ...
  [
    "doris",
    "select bitmap_count(to_bitmap(age)) bc, hll_cardinality(hll_hash(name)) hc from users",
  ],
  ...
  [
    "starrocks",
    "select bitmap_count(to_bitmap(age)) bc, bitmap_union_count(to_bitmap(id)) buc, hll_hash(name) hh from users",
  ],
  ["starrocks", "select get_json_string(data,'$.x') gjs from users"],
  ...
  [
    "risingwave",
    "select tumble_start(created_at, interval '1 hour') ts, tumble_end(created_at, interval '1 hour') te from users",
  ],
  ...
  [
    "singlestore",
    "select json_extract_string(data,'$.x') js, regexp_match(name,'a') rm from users",
  ],
  ...
  ["tidb", "show table regions from users"],
  ["tidb", "select approx_count_distinct(age) acd, group_concat(name) gc from users"],
  ["tidb", "select if(age>30,'senior','junior') tier, convert(a, decimal(10,2)) cv from users"],
  ["solr", "select id, name from users"],
  ["solr", "show tables"],
  ["tableau", "select id, name from users"],
  ["tableau", "show tables"],
  ["tableau", "select top 10 id, name from users order by id"],
  [
    "tableau",
    "select dateadd('day', 1, created_at) da, datediff('day', created_at, current_timestamp) dd from users",
  ],
  [
    "tableau",
    "select iff(age > 30, 'senior', 'junior') tier, string_agg(name, ',') within group (order by id) sa from users",
  ],
  ...
  [
    "postgres",
    "select * from pg_catalog.pg_get_object_address('table', ARRAY['users'], ARRAY[]::text[]) as a",
  ],
  ...
  [
    "postgres",
    "select stddev(age) sd, variance(age) v, mode() within group (order by age) m from users",
  ],
  ...
  [
    "postgres",
    "select regr_count(age, id) rc, regr_avgx(age, id) rax, regr_avgy(age, id) ray from users",
  ],
  ...
  [
    "postgres",
    "select left(name,2) l, right(name,2) r, repeat(name,2) rp, regexp_match(name,'a') rm from users",
  ],
  ...
  [
    "postgres",
    "select overlay(name placing 'x' from 1 for 1) o, ascii(name) a, chr(age) c from users",
  ],
  [
    "postgres",
    "select quote_nullable(name) qn, parse_ident(name) pi, regexp_split_to_table(name, ',') rst from users",
  ],
  [
    "postgres",
    "select width_bucket(age,0,100,10) wb, num_nonnulls(name,age) nn, num_nulls(name,age) nl from users",
  ],
  [
    "postgres",
    "select array_dims(tags) ad, array_ndims(tags) an, jsonb_extract_path_text(data::jsonb,'x') jpt from users",
  ],
  ...
  [
    "postgres",
    "select tags && array['a'] ov, tags @> array['a'] ca, tags <@ array['a','b'] cb from users",
  ],
  ["postgres", "select tags[1] t, tags[1:2] ts from users"],
  ["postgres", "select (id, name) as r from users"],
  [
    "postgres",
    "select name ~* 'a' as regex_match, data #> array['x'] as value, data #>> array['x'] as text_value from users",
  ],
  [
    "postgres",
    "select to_tsvector(name) tv, setweight(to_tsvector(name),'A') sw, websearch_to_tsquery(name) wq from users",
  ],
  ...
  [
    "postgres",
    "select st_setsrid(geom,4326) sg, st_transform(geom,3857) tg, st_makeenvelope(0,0,1,1,4326) env from users",
  ],
  [
    "postgres",
    "select gen_random_bytes(4) grb, digest(name,'sha256') dg, hmac(name,'k','sha256') hm from users",
  ],
  ["postgres", "select encode(decode(name,'hex'),'base64') e from users"],
  [
    "postgres",
    "select greatest(age,1) g, least(age,2) l, make_interval(days => age) mi from users",
  ],
  ["postgres", "select cbrt(age) c, div(age,2) d, factorial(age) f from users"],
  [
    "postgres",
    "select inet_client_addr() ica, inet_server_port() isp, pg_backend_pid() pid, txid_current() tx from users",
  ],
  [
    "postgres",
    "select inet_client_port() icp, current_query() cq, pg_postmaster_start_time() pst from users",
  ],
  [
    "postgres",
    "select obj_description(id) od, col_description(id, age) cdesc, pg_get_expr(name, id) pge from users",
  ],
  [
    "postgres",
    "select date_bin('1 hour', created_at, timestamp '2020-01-01') db, justify_interval(age(created_at)) ji from users",
  ],
  ["postgres", "select isfinite(created_at) inf from users"],
  [
    "postgres",
    "select clock_timestamp() ct, statement_timestamp() st, transaction_timestamp() tt, current_schema() cs from users",
  ],
  ...
  [
    "postgres",
    "merge into users using src on users.id=src.id when matched then update set name=src.name returning users.*, src.name",
  ],
  ...
  [
    "mysql",
    "select json_unquote(json_extract(data, '$.x')) x, json_contains(data, '{}') c, unhex(hex(name)) b, aes_encrypt(name, 'k') e from users",
  ],
  [
    "mysql",
    "select json_valid(data) jv, make_set(age,'a','b') ms, export_set(age,'Y','N') es from users",
  ],
  ...
  [
    "mysql",
    "select json_array(id,name) ja, json_object('id',id) jo, json_type(data) jt, json_length(data) jl from users",
  ],
  [
    "mysql",
    "select json_set(data,'$.x',1) js, json_insert(data,'$.x',1) ji, json_remove(data,'$.x') jr, json_merge_patch(data,data) jm from users",
  ],
  [
    "mysql",
    "select json_merge_preserve(data,data) jmp, json_pretty(data) jp, json_storage_size(data) jss from users",
  ],
  [
    "mysql",
    "select cast(name as json) j, json_keys(data) jk, json_search(data,'one','x') js from users",
  ],
  ["mysql", "select soundex(name) s, space(age) sp, find_in_set(name, 'a,b') f from users"],
  [
    "mysql",
    "select substring_index(name, ',', 1) si, day(d) dy, month(d) mo, year(d) yr, unix_timestamp(created_at) ut from users",
  ],
  [
    "mysql",
    "select date_add(d, interval 1 day) da, timestampdiff(day, created_at, created_at) td from users",
  ],
  [
    "mysql",
    "select curdate() cd, curtime() ct, utc_date() ud, utc_time() ut, from_unixtime(age) fu, sec_to_time(age) st, time_to_sec(curtime()) ts from users",
  ],
  ...
  [
    "mysql",
    "select bit_count(age) bc, connection_id() cid, last_insert_id() li, is_ipv4(name) ip4, is_ipv6(name) ip6 from users",
  ],
  [
    "mysql",
    "select stddev_pop(age) sp, var_pop(age) vp, bit_and(age) ba, bit_or(age) bo from users",
  ],
  ...
  [
    "sqlite",
    "select highlight(email, 1, '[', ']') h, snippet(email, 1, '[', ']', '...', 10) s, bm25(email) b from email where email match 'x'",
  ],
  ...
  [
    "sqlite",
    "select changes() ch, total_changes() tc, last_insert_rowid() lr, sqlite_version() sv",
  ],
  ...
  [
    "snowflake",
    "select object_construct('id', id, 'name', name) o, array_construct(id, age) a from users",
  ],
  [
    "snowflake",
    "select parse_json(name) j, check_json(name) c, is_null_value(parse_json(name)) n from users",
  ],
  [
    "snowflake",
    "select as_array(parse_json(name)) aa, as_object(parse_json(name)) ao, as_varchar(parse_json(name)) av from users",
  ],
  ["snowflake", "select system$typeof(name) st, typeof(parse_json(name)) tf from users"],
  [
    "snowflake",
    "select current_version() cv, current_account() ca, current_region() cr, current_role() role from users",
  ],
  ...
  [
    "snowflake",
    "select split(name, ',') s, array_size(split(name,',')) n, get(parse_json(name),'x') g from users",
  ],
  ["snowflake", "select object_keys(parse_json(name)) ok from users"],
  [
    "snowflake",
    "select object_insert(object_construct('id', id),'x',1) oi, json_array_append(data,'$.x',1) jaa, json_array_insert(data,'$.x',1) jai, json_array_contains(data,'x') jac from users",
  ],
  [
    "snowflake",
    "select array_flatten(array_construct(tags)) af, array_compact(tags) ac, array_intersect(tags,tags) ai, array_union(tags,tags) au, array_except(tags,tags) ae from users",
  ],
  [
    "snowflake",
    "select array_contains_all(tags,tags) aca, array_contained_by(tags,tags) acb, array_overlaps(tags,tags) ao from users",
  ],
  [
    "snowflake",
    "select array_zip(tags, array_construct(age)) az, array_all(tags, x -> x is not null) aa, array_any(tags, x -> x is not null) anyv from users",
  ],
  [
    "snowflake",
    "select to_varchar(age) tv, to_binary(name) tb, md5_binary(name) mb, sha2_binary(name) sb from users",
  ],
  [
    "snowflake",
    "select to_timestamp(name) ts, to_date(name) d, to_time(name) t, to_boolean(name) b from users",
  ],
  ["snowflake", "select try_to_boolean(name) tb, object_construct('id', id) obj from users"],
  [
    "snowflake",
    "select datediff(day, created_at, created_at) dd, dateadd(day, 1, created_at) da, last_day(d) ld from users",
  ],
  ...
  [
    "snowflake",
    "select to_geography(name) g, to_geometry(name) gm, st_distance(geog, geog) dist from users",
  ],
  [
    "snowflake",
    "select cosine_distance(geom, geom) cd, euclidean_distance(geom, geom) ed, manhattan_distance(geom, geom) md from users",
  ],
  ["snowflake", "select hash(name) h, hash_agg(name) ha, uuid_string() u from users"],
  [
    "snowflake",
    "select try_to_decfloat(name) td, ai_agg(name, 'summarize') aa, ai_classify(name, ['a','b']) ac, ml_forecast(age) mf from users",
  ],
  [
    "snowflake",
    "select try_base64_decode_string(name) s, try_base64_decode_binary(name) b from users",
  ],
  [
    "snowflake",
    "select base64_encode(name) e, base64_decode_string(name) ds, base64_decode_binary(name) db from users",
  ],
  [
    "snowflake",
    "select compress(name) c, decompress_binary(name) db, decompress_string(name) ds, parse_url(name) pu, check_xml(name) cx from users",
  ],
  ...
  [
    "bigquery",
    "select * from generate_timestamp_array(timestamp '2020-01-01', timestamp '2020-01-02', interval 1 day) as ts",
  ],
  ...
  [
    "bigquery",
    "select ieee_divide(age, 2) idv, is_inf(ieee_divide(age,0)) inf, is_nan(ieee_divide(age,0)) nan from users",
  ],
  ...
  [
    "bigquery",
    "select json_query_array(data,'$.x') jqa, json_value_array(data,'$.x') jva from users",
  ],
  ["bigquery", "select json_strip_nulls(data) jsn, stringify_json(data) sj from users"],
  [
    "bigquery",
    "select generate_embedding(model => 'm', content => name) ge, ai_classify(name, ['a','b']) ac from users",
  ],
  [
    "bigquery",
    "select generate_uuid() u, farm_fingerprint(name) f, to_base64(from_base64(name)) b from users",
  ],
  [
    "bigquery",
    "select starts_with(name,'a') sw, ends_with(name,'z') ew, regexp_extract_all(name,'[a-z]') xs from users",
  ],
  ["bigquery", "select ascii(name) a, chr(age) c, code_points_to_string([65,66]) cps from users"],
  [
    "bigquery",
    "select to_code_points(name) tcp, split(name, ',') s, byte_length(name) bl from users",
  ],
  [
    "bigquery",
    "select date(d) dd, timestamp(created_at) ts, datetime(created_at) dt, time(created_at) tm from users",
  ],
  ["bigquery", "select unix_seconds(created_at) us, timestamp_seconds(age) tse from users"],
  [
    "bigquery",
    "select array_length(tags) l, array_reverse(tags) r, array_concat(tags,tags) c from users",
  ],
  [
    "bigquery",
    "select array_to_string(tags, ',') ats, array_first(tags) af, array_last(tags) al from users",
  ],
  ...
  ["tsql", "select id, name from users for json path"],
  ["tsql", "select id, name from users for xml raw"],
  ["tsql", "select @@spid spid, db_name() db, host_name() host, app_name() app"],
  [
    "tsql",
    "select getdate() gd, sysdatetime() sdt, sysutcdatetime() sudt, datename(month,created_at) dn from users",
  ],
  [
    "tsql",
    "select object_name(id) oname, schema_name(id) sname, db_id() dbid, object_id('users') oid from users",
  ],
  ...
  [
    "tsql",
    "select checksum(name) cs, binary_checksum(name) bc, patindex('%a%', name) pi from users",
  ],
  ...
  [
    "oracle",
    "select regexp_count(name,'a') rc, to_clob(name) tc, rawtohex(name) rh, hextoraw(name) hr from users",
  ],
  ...
  [
    "oracle",
    "select * from users match_recognize (partition by dept order by created_at measures match_number() as match_no, classifier() as cls all rows per match pattern (A+) define A as age > 0) mr",
  ],
  ...
  [
    "duckdb",
    "select contains(tags,'a') c, list_contains(tags,'a') lc, list_extract(tags,1) le from users",
  ],
  [
    "duckdb",
    "select list_apply(tags, x -> upper(x)) la, map_extract(attrs,'a') me, map_contains(attrs,'a') mc from users",
  ],
  [
    "duckdb",
    "select name.lower() l, name.upper() u, tags.list_contains('a') c, tags.list_extract(1) e from users",
  ],
  [
    "duckdb",
    "select list_has(tags,'a') lh, list_has_all(tags,tags) lha, list_has_any(tags,tags) lhan from users",
  ],
  ["duckdb", "select list_resize(tags, 5) lr from users"],
  [
    "duckdb",
    "select list_distinct(tags) ld, list_sort(tags) ls, list_grade_up(tags) lgu, list_select(tags, [1]) lsel from users",
  ],
  [
    "duckdb",
    "select array_pop_back(tags) pb, array_pop_front(tags) pf, array_push_back(tags,'x') pback, array_push_front(tags,'x') pfront from users",
  ],
  [
    "duckdb",
    "select prefix(name,'a') p, suffix(name,'z') s, list_position(tags,'x') lp, list_unique(tags) lu from users",
  ],
  [
    "duckdb",
    "select mad(age) mad, sem(age) sem, skewness(age) sk, quantile_disc(age, 0.5) qd from users",
  ],
  [
    "duckdb",
    "select array_filter(tags, x -> x <> 'a') af, array_reduce(ints, (x,y) -> x + y) ar from users",
  ],
  [
    "duckdb",
    "select epoch(created_at) e, epoch_ms(created_at) ems, make_date(2020,1,1) md from users",
  ],
  [
    "duckdb",
    "select last_day(d) ld, list_slice(tags, 1, 2) ls, list_reverse(tags) lr, list_concat(tags,tags) lc from users",
  ],
  ...
  [
    "duckdb",
    "select json_extract(data,'$.x') je, json_extract_string(data,'$.x') jes, json_type(data) jt from users",
  ],
  ...
  [
    "clickhouse",
    "select version() v, currentDatabase() db, currentUser() u, sipHash64(name) sh, bitCount(age) bc from users",
  ],
  ["clickhouse", "select generateUUIDv4() u from users"],
  [
    "clickhouse",
    "select toInt32(name) i, toFloat64(name) f, toUUID(name) u, toBool(age) b from users",
  ],
  ["clickhouse", "select has(tags,'a') h, length(tags) l, arrayJoin(tags) tag from users"],
  [
    "clickhouse",
    "select arrayConcat(tags,tags) ac, arrayDistinct(tags) ad, arraySlice(tags,1,2) aslice, arrayReverse(tags) ar from users",
  ],
  ["clickhouse", "select mapContains(attrs,'a') mc from users"],
  [
    "clickhouse",
    "select arrayStringConcat(tags,',') asc, empty(tags) e, notEmpty(tags) ne from users",
  ],
  ...
  [
    "clickhouse",
    "select replaceAll(name,'a','b') ra, arrayMap(x -> upper(x), tags) am, arrayFilter(x -> x != 'a', tags) af from users",
  ],
  [
    "clickhouse",
    "select JSONExtractString(name,'id') js, JSONExtractInt(name,'id') ji, JSONHas(name,'id') jh from users",
  ],
  [
    "clickhouse",
    "select JSONLength(name) jl, JSONType(name) jt, toDateTime(name) tdt, toUnixTimestamp(created_at) tut from users",
  ],
  [
    "clickhouse",
    "select toStartOfDay(created_at) sd, formatDateTime(created_at,'%F') fd from users",
  ],
  ["clickhouse", "select format_datetime(current_timestamp, 'yyyy') fy from users"],
  [
    "clickhouse",
    "select today() t, yesterday() y, toStartOfWeek(created_at) sw, subtractDays(d,1) sd from users",
  ],
  [
    "clickhouse",
    "select addDays(d, 1) ad, toYear(d) y, lowerUTF8(name) l, upperUTF8(name) u from users",
  ],
  [
    "clickhouse",
    "select row_number() over(order by age) rn, lagInFrame(name) over(order by id) l from users",
  ],
  ...
  [
    "trino",
    "select any_match(tags, x -> x = 'a') a, all_match(tags, x -> x <> '') b, none_match(tags, x -> x = 'z') n from users",
  ],
  [
    "trino",
    "select json_extract(data,'$.x') j, json_extract_scalar(data,'$.x') s, contains(tags,'a') c from users",
  ],
  ...
  [
    "trino",
    "select map_filter(attrs, (k,v) -> v > 0) mf, transform_values(attrs, (k,v) -> v + 1) tv, map_concat(attrs, attrs) mc from users",
  ],
  ["trino", "select tags[1] tag, data['x'] j from users"],
  [
    "trino",
    "select strpos(name,'a') sp, codepoint(name) cp, chr(age) ch, regexp_like(name,'a') rl, regexp_position(name,'a') rp from users",
  ],
  [
    "trino",
    "select url_extract_host(name) h, url_encode(name) ue, from_utf8(to_utf8(name)) fu from users",
  ],
  [
    "trino",
    "select url_extract_protocol(name) up, url_extract_path(name) path, json_array_length(data) jal from users",
  ],
  [
    "trino",
    "select last_day_of_month(d) lm, lpad(name, 5, 'x') lp, rpad(name,5,'x') rp from users",
  ],
  ...
  [
    "trino",
    "select approx_set(name) aset, set_digest(name) sd, numeric_histogram(10, age) nh from users",
  ],
  ...
  [
    "spark",
    "select array_position(tags,'x') ap, array_distinct(tags) ad, array_remove(tags,'x') ar from users",
  ],
  ["spark", "select exists(tags, x -> x = 'a') e, forall(tags, x -> x <> '') f from users"],
  ["spark", "select map_concat(attrs, map('b',2)) m from users"],
  [
    "spark",
    "select map_filter(attrs, (k,v) -> v > 0) mf, transform_values(attrs, (k,v) -> v + 1) tv, map_from_arrays(tags, ints) mfa from users",
  ],
  ...
  [
    "spark",
    "select from_json(name, 'id INT, name STRING') j, to_json(named_struct('id', id)) tj from users",
  ],
  ["spark", "select get_json_object(name,'$.id') g, json_tuple(name,'id') jt from users"],
  [
    "spark",
    "select collect_list(name) cl, collect_set(name) cs, percentile_approx(age, 0.5) pa from users",
  ],
  ...
  [
    "spark",
    "select current_database() db, current_user() u, version() v, hash(name) h, xxhash64(name) xx from users",
  ],
  ...
  [
    "hive",
    "select current_database() db, current_user() u, version() v, pmod(age,2) pm, conv(name,10,16) cv from users",
  ],
...
const schemaTrackingCases = [
  [
    "postgres",
    "create schema app; create table app.people(id int, name text); select id, name from app.people",
  ],
  [
    "postgres",
    "create table t(id int generated always as identity, name text default 'x', created_at timestamp not null); select id, name, created_at from t",
  ],
  ...
  [
    "postgres",
    "create table t(id int, name text); create materialized view v(a, b) as select * from t; select a, b from v",
  ],
  [
    "postgres",
    "create table t as select id, name from users; alter table t rename to t2; select name from t2",
  ],
  [
    "postgres",
    "create table app.t(id int); alter schema app rename to app2; select id from app2.t",
  ],
  ["postgres", "create table t(id int); alter table t add column name text; select name from t"],
  [
    "postgres",
    "create table t(id int, name text); alter table t rename column name to full_name; select full_name from t",
  ],
  [
    "postgres",
    "create table t(id int, name text); alter table t drop column name; select id from t",
  ],
  ["postgres", "create table t (like users, extra int); select name, extra from t"],
  [
    "postgres",
    "create domain positive_int as int check (value > 0); create table t(id positive_int); select id from t",
  ],
  [
    "postgres",
    "create type mood as enum ('sad','ok','happy'); create table t(m mood); select m from t",
  ],
  ["postgres", "create type pair as (id int, name text); create table t(p pair); select p from t"],
  [
    "postgres",
    "create table people(id int, name text); alter table people set schema app; select id, name from app.people",
  ],
  [
    "postgres",
    "create table people(id int, name text); create view app_people as select id, name from people; alter view app_people rename to people_view; select name from people_view",
  ],
  [
    "postgres",
    "create table people(id int, name text); create synonym p for people; select name from p",
  ],
  [
    "postgres",
    "create function f(x int) returns int language sql as $$ select x $$; select f(age) from users",
  ],
  [
    "postgres",
    "create function f(x int) returns int language sql as $$ select x $$; create table t as select f(age) as v from users; select v from t",
  ],
  [
    "postgres",
    "create function f(x int) returns int language sql as $$ select x $$; create view v as select f(age) as n from users; select n from v",
  ],
  [
    "postgres",
    "create function people() returns table(id int, name text) language sql as $$ select 1, 'a' $$; select id, name from people()",
  ],
  [
    "postgres",
    "create function people() returns table(id int, name text) language sql as $$ select 1, 'a' $$; select * from people()",
  ],
  ["mysql", "create procedure p() begin select id, name from users; end; call p()"],
  ["bigquery", "create temp function f(x int64) as (x + 1); select f(age) from users"],
  ["bigquery", "create function f(x int64) returns int64 as (x + 1); select f(age) from users"],
  [
    "snowflake",
    "create function f(x number) returns number as $$ x + 1 $$; select f(age) from users",
  ],
  [
    "snowflake",
    "create procedure p() returns table(id number, name string) language sql as $$ select id, name from users $$; call p()",
  ],
  ...
  [
    "tsql",
    "create table t(id int identity(1,1) primary key, name nvarchar(20) not null, amount decimal(10,2)); select id, name, amount from t",
  ],
  ...
  [
    "mysql",
    "create table t(id int auto_increment primary key, name varchar(20) not null, amount decimal(10,2)); select id, name, amount from t",
  ],
  ...
  [
    "mysql",
    "create table t(id int, name varchar(20)); alter table t change name full_name varchar(30); select full_name from t",
  ],
  [
    "sqlite",
    "create table t(id int, name text); alter table t rename column name to full_name; select full_name from t",
  ...
  [
    "sqlite",
    "create virtual table email using fts5(sender, title, body); select sender, title from email",
  ],
  [
    "bigquery",
    "create table t(id int64, name string, amount numeric); select id, name, amount from t",
  ],
  [
    "bigquery",
    "create table t(id int64); alter table t add column name string; select name from t",
  ],
  ...
  [
    "oracle",
    "create global temporary table t(id number, name varchar2(20)) on commit preserve rows; select id, name from t",
  ],
  ...
  [
    "duckdb",
    "create table t(id int, name varchar); alter table t rename column name to full_name; select full_name from t",
  ],
  ["duckdb", "create macro add_one(x) as x + 1; select add_one(age) from users"],
  [
    "duckdb",
    "create macro people_macro() as table select id, name from users; select name from people_macro()",
  ],
...
function supportedTableFunctionCase(dialect: string, sql: string): boolean {
  ...
  if (/read_csv|read_json/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("fileColumns");
  if (/\b(?:file|url|s3|generateRandom)\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("schemaStringTableFunctions");
  if (/\bnumbers\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("numbers");
  if (/\bsequence\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("sequence");
  if (/\bgenerate_series\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("generateSeries");
  if (/\brange\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("range");
  if (/\bgenerate_array\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("generateArray");
  if (/\bstack\s*\(/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("stack");
  if (/sys\.odcivarchar2list/i.test(sql))
    return config.dynamicTableFunctions.enabledHandlers.includes("oracleCollection");
  return true;
}
...
function supportedStaticResultCase(dialect: string, sql: string): boolean {
  ...
  const metadataNames = new Set(
    config.metadata.builtinSchemaTables.map((table) =>
      (table.schema ? `${table.schema}.${table.name}` : table.name).toLowerCase(),
    ),
  );
  const tableFunctionNames = new Set(
    Object.keys(config.tableFunctions).map((name) => name.toLowerCase()),
  );
  if (metadataNames.has(referencedName))
    return selectedMetadataColumnsAreConfigured(config, sql, referencedName);
  if (tableFunctionNames.has(first)) return true;
  if (second && metadataNames.has(second))
    return selectedMetadataColumnsAreConfigured(config, sql, second);

  return !/^(?:information_schema|pg_catalog|sys|system|account_usage|svv|stl|exa_|all_|dba_|user_|v\$)/i.test(
    referencedName,
  );
}

function selectedMetadataColumnsAreConfigured(
  config: DialectConfig,
  sql: string,
  tableName: string,
): boolean {
  const table = config.metadata.builtinSchemaTables.find(
    (candidate) =>
      (candidate.schema
  ...
  for (const dialect of getDialects()) {
    it(`describes a schema-backed SELECT for supported dialect: ${dialect}`, async () => {
      ...
      assert.deepStrictEqual(
        result.columns.map((column) => [column.name, column.source]),
        [
          ["id", "users.id"],
          ["name", "users.name"],
        ],
...
function representativeSqlCases(): Array<[string, string]> {
  ...
  return [
    ...
    ...dialects.flatMap((dialect) =>
      returningDmlCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      schemaProducingDdlCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      structuralSelectCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      tableFunctionCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      broadNoResultCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      bindPlaceholderCases.map(({ sql }): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      expressionShapeCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      aggregateWindowCases.map((sql): [string, string] => [dialect, sql]),
    ),
    ...dialects.flatMap((dialect) =>
      selectModifierCases.map((sql): [string, string] => [dialect, sql]),
    ),
  ];
}
