...
const staticResultCases = [
  ...
  ['teradata', 'select csum(age) over (order by id) csum, mavg(age) over (order by id rows 3 preceding) mavg from users'],
  ...
  ['starrocks', 'select bitmap_count(to_bitmap(age)) bc, bitmap_union_count(to_bitmap(id)) buc, hll_hash(name) hh from users'],
  ...
  ['tableau', "select dateadd('day', 1, created_at) da, datediff('day', created_at, current_timestamp) dd from users"],
  ['tableau', "select iff(age > 30, 'senior', 'junior') tier, string_agg(name, ',') within group (order by id) sa from users"],
...
const schemaTrackingCases = [
  ...
  ['postgres', 'create table t(id int, name text); create materialized view v(a, b) as select * from t; select a, b from v'],
